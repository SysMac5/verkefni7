...
function formatProduct(product, quantity = undefined) {
  if (quantity != undefined && validateInteger(quantity)) {
...
function cartInfo(cart) {
  ...
  for (var i = 0; i < cart.lines.length; i++) {
    if (cart.lines[i].quantity > 1) {
      info += formatProduct(cart.lines[i].product, cart.lines[i].quantity) + '\n';
    } else {
      info += formatProduct(cart.lines[i].product) + '\n';
    }
    priceSum += cart.lines[i].quantity * cart.lines[i].product.price;
  }
  info += `Samtals: ${formatPrice(priceSum)}\n`
  return info;
}
...
function showProducts() {
  ...
  for (var i = 0; i < products.length; i++) {
    productsAsString += `#${products[i].id} ${products[i].title} — ${products[i].description} - ${formatPrice(products[i].price)} \n`;
  }
  console.info(productsAsString);
}
...
function addProductToCart() {
  const idAsString = prompt('Auðkenni vöru sem bæta á við í körfu:');
  if (!idAsString) {
  ...
  const quantityAsString = prompt('Fjöldi af vöru sem bæta á við í körfu:');
  if (!quantityAsString) {
  ...
  const indexInCart = cart.lines.findIndex(line => line.product.id === id);
  if (indexInCart === -1) {
    cart.lines.push(cartLine);
  } else {
    cart.lines[indexInCart].quantity = quantity;
  }

  if (cartLine.quantity > 1) {
    console.info(`Vöru bætt við körfu:\n${formatProduct(cartLine.product, cartLine.quantity)}`);
  } else {
    console.info(`Vöru bætt við körfu:\n${formatProduct(cartLine.product)}`);
  }
}
...
function checkout() {
  ...
  if (!address) {
    console.error('Heimilsfang má ekki vera tómt.');
    return;
  }
